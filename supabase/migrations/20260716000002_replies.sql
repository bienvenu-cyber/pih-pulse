-- ============================================================
-- Réponses / Threads / Mentions / Épinglage / Impact feedback
-- UI: Réponses (posts) · Questions (missions) · Échanges (projets)
-- ============================================================
...
create index if not exists idx_replies_pinned on public.replies (ref_type, ref_id, pinned_at desc nulls last);
...
-- Awards anti-farm : useful (+5) et pin (+8) 1× par reply
...
create index if not exists idx_reply_awards_reply on public.reply_awards (reply_id);
...
create policy "reply_awards_select"
  on public.reply_awards for select using (true);

drop policy if exists "reply_awards_insert_auth" on public.reply_awards;
create policy "reply_awards_insert_auth"
  on public.reply_awards for insert
