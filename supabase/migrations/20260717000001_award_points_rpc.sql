-- ============================================================
-- S1 — award_points RPC + fermer insert client sur reputation_logs
-- ============================================================
-- Les points (Élan) ne passent plus par INSERT direct client.
-- Trigger update_profile_reputation inchangé.

-- Idempotence des awards (clé fournie par le client / métier)
...
-- Qui a attribué (audit / rate-limit)
...
/**
 * award_points — SECURITY DEFINER (bypass RLS insert)
 * - auth requis
 * - |points| entre 1 et 300
 * - rate limit : 150 awards / heure / awarded_by
 * - idempotency_key optionnelle (unique globale)
 */
...
begin
  ...
  if v_key is not null then
    ...
    select log_id into v_id
    from public.reputation_idempotency
    where key = v_key;
  ...
  if coalesce(v_hour_count, 0) >= 150 then
...
-- Fermer INSERT client direct (seule la RPC security definer écrit)
drop policy if exists "Authenticated can insert reputation logs." on public.reputation_logs;

-- Lecture historique inchangée (public select déjà en place)
-- Select logs : garder lisible pour profil impact
